...
import { connect, cutImgUrl } from '../../../utils';
...
class CustomTabBarBottom extends React.PureComponent {
  ...
  render() {
    ...
    const url = this.props.background ? this.props.background : '';
    Log('====================================');
    Log();
    Log('====================================');

    return (this.props.showTab ? <Animated.View style={animateStyle}>
        <SafeAreaView
          style={tabBarStyle}
          forceInset={{ bottom: 'always', top: 'never' }}
        >
            {url ? <ImageBackground source={{uri: cutImgUrl(url)}} style={{width, height: '100%', flexDirection: 'row'}}>
          {routes.map((route, index) => {
            ...
            return (
              ...
              >
                ...
                :<View source={{uri: cutImgUrl(url)}} style={{width, height: '100%', flexDirection: 'row'}}>
          {routes.map((route, index) => {
            const focused = index === navigation.state.index;
            const scene = { route, index, focused };
            const onPress = getOnPress(previousScene, scene);
...
export default connect(({home: {bottomIconConfig : {iconImageConfig}, showTab}})=> ({iconImageConfig, showTab}))(CustomTabBarBottom);
