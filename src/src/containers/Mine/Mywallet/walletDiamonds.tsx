...
interface IState {
  ...
  class WalletIntegral extends React.Component<IState & INavigation> {
    public state:IState;
    constructor(props) {
      super(props);
      this.state = {
    ...
    render():JSX.Element {
      return (
        <View>
          ...
          >
            <LinearGradient
            colors={['#C070FE', '#855AFF']}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 1 }}
            >
              <View style={styles.topCont}>
                <View style={{paddingLeft:20}}>
                  <Text style={{color:'#fff',fontSize:14,marginTop:34,height:20}}>
                  <Image 
                  style={styles.Igimg}
                  source={require('../../../images/cuizuan.png')}
                  />&nbsp;&nbsp;当前钻石</Text>
                  <Text style={{color:'#fff',marginTop:6}}><Text  style={{fontSize:36}}>{this.state.walletDate.usableDiamond}</Text>个</Text>
                </View>
            ...
            {
                this.state.emptys?
                (<Text style={{color:'#999',fontSize:14,marginTop:110,textAlign:'center'}}>暂无钻石明细～</Text>)
                :
                <FlatList 
