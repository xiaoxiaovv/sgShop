import {superMaketService} from '../service'
...
export default {
  ...
  effects: {
    * getTabbar({payload}, {call, put, select}) {
      try {
        const res = yield call(superMaketService.fetchProductCates);
    ...
    * getAdornHomeBanner({payload}, {call, put}) {
      try {
        const res = yield call(superMaketService.fetchAdornHomeBanner);
    ...
    * getTopRecommendProducts({payload: {parentCateId}}, {call, put, select}) {
      try {
        ...
        const res = yield call(superMaketService.fetchTopRecommendProducts, {
    ...
    * getLowRecommendProducts({payload: {parentCateId}}, {call, put, select}) {
      try {
        ...
        const cityId = address.cityId;
        const provinceId = address.provinceId;
        const res = yield call(superMaketService.fetchLowRecommendProducts, {
          parentCateId: parentCateId,
          provinceId,
          cityId,
          districtId,
